...
export async function createCompetitorAction(formData: FormData) {
  ...
  await addCompetitor(workspaceId, name, domain);
  revalidatePath("/app");
}
...
export async function createSourceAction(formData: FormData) {
  ...
  await addSource(workspaceId, competitorId, url, kind, label);
  revalidatePath("/app");
}
...
export async function fetchAndDigestAction(formData: FormData) {
  ...
  await fetchAllForCompetitor(workspaceId, competitorId);
...
export async function createWebhookAction(formData: FormData) {
  ...
  const label = String(formData.get("label") ?? "").trim() || titleCase(kind);
  if (!competitorId || !url) return;
  await addWebhook(workspaceId, competitorId, url, kind, label);
  revalidatePath("/app");
  revalidatePath("/app/digests");
}
...
export async function removeWebhookAction(formData: FormData) {
  ...
  if (!id) return;
  await removeWebhook(workspaceId, id);
  revalidatePath("/app");
  revalidatePath("/app/digests");
}
...
export async function removeDigestAction(formData: FormData) {
  ...
  const id = Number(formData.get("id"));
  if (!id) return;
  await removeDigest(workspaceId, id);
  revalidatePath("/app");
  revalidatePath("/app/digests");
}
...
export async function testWebhooksAction(formData: FormData) {
  ...
  const competitorId = Number(formData.get("competitor_id"));
  if (!competitorId) return;
  const latest = (await listDigests(workspaceId, competitorId))[0];
  if (!latest) return;
  await deliverDigest(workspaceId, competitorId, latest.id);
  revalidatePath("/app");
  revalidatePath("/app/digests");
}
